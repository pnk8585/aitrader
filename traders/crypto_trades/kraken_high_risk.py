"""kraken_high_risk.py — High-risk momentum breakout strategy on Kraken.

Port of kraken_momentum.py with aggressive high-risk config overrides.
Entry: daily >= 4% or hourly >= 3%.
Exit: trailing TP, profit lock, stop-loss, breakeven, stale rotation, hard TP cap.

SHARED-WALLET NOTE
------------------
This script shares the Kraken EUR wallet with kraken_pullback.py. To avoid the
two strategies fighting over the same coin:
  - high-risk only EXITS positions it recorded in its own trading_state
    (EXCHANGE_NAME = "kraken-high-risk", paper- prefix in paper mode);
    it never sells a coin the pullback script is holding.
  - high-risk never ENTERS a coin already present in the Kraken balance (held by
    either strategy), and respects a global cap on total open positions.
In paper mode, exchange key isolation keeps paper trades separate from live.
If ever live, still use coins_held_by_other_bots to avoid wallet collisions.
...
# ---------------------------------------------------------------------------
# Config (from strategies.momentum.config_high_risk)
# ---------------------------------------------------------------------------
ensure_log_dir()
...
HARD_TP_CAP_PCT = MO.HARD_TP_CAP_PCT
...
def run_cycle():
    _clear_atr_cache()
    CRYPTO_PAIRS = HIGH_RISK_PAIRS

    report = {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "status": "success",
        "strategy": "high-risk-momentum",
        "positions_managed": [],
        "scanned_assets": [],
        "action_taken": "NONE",
        "details": "",
    ...
    def finalize():
        ...
        if is_error:
            print(f"❌ Kraken high-risk: {report.get('details', 'unknown error')}")
        elif has_trade:
            ...
            print(f"⚠️ Kraken high-risk {action}: {report.get('details', '')}")
    ...
    for pos in my_positions[:]:
        ...
        # Hard take-profit cap: after standard exit rules say no, force full
        # exit if unrealized exceeds the aggressive cap.
        if not sell and getattr(MO, "HARD_TP_CAP_PCT", None) and unrealized_plpc >= MO.HARD_TP_CAP_PCT:
            sell = True
            reason = f"Take-profit cap ({MO.HARD_TP_CAP_PCT}%)"
    ...
    if not skip and len(all_positions) >= MAX_TOTAL_OPEN:
        ...
        if not skip:
            ...
            if rpnl <= DAILY_LOSS_BREAKER_PCT:
                ...
                should_notify = True
                msg_lines.append(f"🚨 **Kraken high-risk daily loss breaker**: {round(rpnl,2)}% "
    ...
    if not candidates:
        report["action_taken"] = "SKIP"
        report["details"] = "No high-risk momentum breakout setup passed the filters."
        finalize()
        return
    ...
    if _aggressive_mode:
        ...
        # trade. Restore blocking via app_settings.llm.review_mode = 'gate'.
    ...
    if need_rotation:
        ...
        try:
            ...
            _rot_res = res or {}
            should_notify = True
            msg_lines.append(f"🔄 **Περιστροφή (Kraken high-risk)**: Πωλήθηκε στάσιμο "
    ...
    # the old hard-coded 0.50 silently halved every entry. Deploy 100% unless DCA is on.
    _entry_deploy_pct = 0.50 if MO.USE_DCA_ENTRY else 1.00
    ...
    try:
        ...
        should_notify = True
        msg_lines.append(f"🛒 **Αγοράστηκε {symbol} (Kraken high-risk)** "
...
def main():
    ...
    try:
        ...
        print("Another kraken_high_risk cycle is already running — skipping this tick.",
              file=sys.stderr)
        return
    try:
        run_cycle()
        ...
        import traceback
        print(f"🚨 **Kraken high-risk CRASHED**: {e}")
        print(f"ALERT: kraken_high_risk crashed: {e}\n{traceback.format_exc()}",
              file=sys.stderr)
    finally:
        try:
